...
def test_file_trace_storage_store_get_delete_and_stats(tmp_path):
    ...
    td = TraceData(trace_id=trace_id, root_span_id='s1', spans=[span], start_time=datetime.now(), end_time=datetime.now(), duration_ms=12.3, service_count=1, total_spans=1, error_count=0, status='active')
...
def test_query_and_cleanup(tmp_path):
    ...
    td1 = TraceData(trace_id=tid_recent, root_span_id='sr1', spans=[span1], start_time=datetime.now(), end_time=datetime.now(), duration_ms=5.0, service_count=1, total_spans=1, error_count=1, status='active')

    # Old trace (10 days ago)
    tid_old = 'o1'
    old_start = datetime.now() - timedelta(days=10)
    span2 = TraceSpan(span_id='so1', trace_id=tid_old, parent_span_id=None, name='op', kind='internal', start_time=old_start, end_time=old_start, duration_ms=3.0, status='ok', attributes={}, events=[], service_name='svcB', agent_name='agent', operation='op')
    td2 = TraceData(trace_id=tid_old, root_span_id='so1', spans=[span2], start_time=old_start, end_time=old_start, duration_ms=3.0, service_count=1, total_spans=1, error_count=0, status='active')
...
def test_cleanup_old_traces(tmp_path):
    ...
    old_td = TraceData(trace_id='t_old', root_span_id='old', spans=[old_span], start_time=(datetime.now() - timedelta(days=10)), end_time=(datetime.now() - timedelta(days=10)), duration_ms=1.0, service_count=1, total_spans=1, error_count=0)

    recent_span = make_span('r1', duration=2)
    recent_td = TraceData(trace_id='t_new', root_span_id='r1', spans=[recent_span], start_time=datetime.now(), end_time=datetime.now(), duration_ms=2.0, service_count=1, total_spans=1, error_count=0)
