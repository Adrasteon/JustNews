...
Purpose: Remove the deprecated `pkg_resources.declare_namespace` call from the
`google.rpc` package that ships with `googleapis-common-protos`. The upstream
module still relies on Setuptools' legacy namespace helper, which emits a
...
Whenever the Python environment is rebuilt (new conda env, dependency upgrade,
CI image refresh, etc.), rerun:

```bash
conda run --name justnews-py312 python scripts/vendor_patches/apply_google_rpc_namespace_patch.py
```
