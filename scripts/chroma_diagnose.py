...
def main():
    ...
    parser.add_argument(
        "--autocreate",
        action="store_true",
        help="Try to auto-create default tenant and articles collection via HTTP API",
    )
    ...
    if canonical_host and canonical_port:
        print(
            f"🔎 Diagnosing ChromaDB at {host}:{port} (canonical configured as {canonical_host}:{canonical_port})\n"
        )
    else:
