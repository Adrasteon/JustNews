...
This runs environment checks, optional DB probes, a reset+fresh start (GPU orchestrator → MCP Bus → agents), provisions monitoring if needed, and performs a consolidated health check.
...
2. Real startup (recommended):

```bash
sudo ./infrastructure/systemd/canonical_system_startup.sh
```

3. Optional manual orchestrator-first flow (when you need fine-grained control):
