...
def run_manage_cmd(cmd_args):
    cmd = [sys.executable, os.path.join(os.getcwd(), 'agents', 'publisher', 'manage.py')] + cmd_args
...
def test_publisher_api_and_metrics():
    ...
    server_proc = subprocess.Popen([
        ...
        'runserver', f'127.0.0.1:{server_port}'
    ...
    try:
        ...
        while time.time() - start < 8:
            try:
                with urllib.request.urlopen(f'http://127.0.0.1:{server_port}/', timeout=1) as r:
                    ready = True
                    break
            except Exception:
                time.sleep(0.25)
        ...
        req = urllib.request.Request(f'http://127.0.0.1:{server_port}/api/publish/', data=json.dumps(payload).encode('utf-8'), headers={'Content-Type': 'application/json', 'X-API-KEY': 'ci-key'})
        ...
        with urllib.request.urlopen(f'http://127.0.0.1:{server_port}/api/metrics/', timeout=5) as r:
            assert r.status == 200
            data = json.loads(r.read().decode('utf-8'))
            assert data.get('success', 0) >= 1

        # Ensure the Prometheus metrics endpoint is available and reports publishing metric name
        with urllib.request.urlopen(f'http://127.0.0.1:{server_port}/metrics/', timeout=5) as r:
            body = r.read().decode('utf-8')
