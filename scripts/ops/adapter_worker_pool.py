...
def _orchestrator_base(url: str | None) -> str:
    if not url:
        raise RuntimeError('GPU orchestrator URL is required for remote mode (set --orchestrator-url or GPU_ORCHESTRATOR_URL)')
    return url.rstrip('/')


def _remote_start_pool(base_url: str, admin_key: str | None, payload: dict[str, Any]) -> dict[str, Any]:
    requests = _lazy_requests()
    resp = requests.post(f"{base_url}/workers/pool", headers=_orchestrator_headers(admin_key), json=payload, timeout=20)
    if resp.status_code >= 400:
        raise RuntimeError(f"Orchestrator start failed ({resp.status_code}): {resp.text}")
    return resp.json()


def _remote_list_pools(base_url: str, admin_key: str | None) -> list[dict[str, Any]]:
    requests = _lazy_requests()
    resp = requests.get(f"{base_url}/workers/pool", headers=_orchestrator_headers(admin_key), timeout=10)
    if resp.status_code >= 400:
        raise RuntimeError(f"Orchestrator list failed ({resp.status_code}): {resp.text}")
    return resp.json()


def _remote_stop_pool(base_url: str, admin_key: str | None, pool_id: str) -> dict[str, Any]:
    requests = _lazy_requests()
    resp = requests.delete(f"{base_url}/workers/pool/{pool_id}", headers=_orchestrator_headers(admin_key), timeout=10)
    if resp.status_code >= 400:
        raise RuntimeError(f"Orchestrator stop failed ({resp.status_code}): {resp.text}")
    return resp.json()
...
def main(argv=None):
    ...
    p.add_argument('--agent', type=str, default=None, help='Optional logical agent name used for orchestrator audit')
    p.add_argument('--pool-id', type=str, default=None, help='Explicit pool identifier when using remote mode')
    p.add_argument('--variant', type=str, default=None, help='Hint for orchestrator loading strategy (fp16, bnb-4bit-qlora, etc)')
    p.add_argument('--orchestrator-url', type=str, default=os.environ.get('GPU_ORCHESTRATOR_URL'), help='GPU orchestrator base URL (enables remote mode when set)')
    p.add_argument('--admin-key', type=str, default=os.environ.get('GPU_ORCHESTRATOR_ADMIN_KEY') or os.environ.get('ADMIN_API_KEY'), help='Admin API key for orchestrator authentication')
    p.add_argument('--remote', action='store_true', help='Delegate pool management to the GPU orchestrator instead of spawning local processes')
    p.add_argument('--list-remote', action='store_true', help='List remote worker pools and exit (requires orchestrator URL + admin key)')
    p.add_argument('--stop-remote', type=str, default=None, help='Stop a remote pool by id and exit')
