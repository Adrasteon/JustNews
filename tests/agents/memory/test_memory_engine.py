...
async def memory_engine(mock_db_service, mock_embedding_model):
    ...
    with patch('database.utils.migrated_database_utils.create_database_service', return_value=mock_db_service), \
         patch('agents.memory.tools.get_embedding_model', return_value=mock_embedding_model):
...
class TestMemoryEngineInitialization:
    ...
    async def test_initialization_success(self, mock_db_service, mock_embedding_model):
        ...
        with patch('agents.memory.memory_engine.create_database_service', return_value=mock_db_service), \
...
class TestMemoryEngineArticleOperations:
    ...
    @pytest.mark.asyncio
    async def test_save_article_success(self, memory_engine, mock_db_service, mock_embedding_model):
...
class TestMemoryEngineTrainingOperations:
    ...
    async def test_log_training_example_success(self, memory_engine, mock_db_service):
        ...
        result = memory_engine.log_training_example(task, input_data, output_data, critique)
        assert result["status"] == "logged"

    @pytest.mark.asyncio
    async def test_log_training_example_db_failure(self, memory_engine, mock_db_service):
...
class TestMemoryEngineStatistics:
    ...
    async def test_get_sources(self, memory_engine, mock_db_service):
        ...
        mock_cursor = MagicMock()
        mock_cursor.fetchall.return_value = [
            {"id": 1, "url": "http://source1.com", "domain": "source1.com", "name": "Source 1"},
            {"id": 2, "url": "http://source2.com", "domain": "source2.com", "name": "Source 2"}
        ]
        mock_db_service.mb_conn.cursor.return_value = mock_cursor
