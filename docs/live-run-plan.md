...
| 2. Ingestion & normalization | 🟢 Archive → Stage B wired | `archive.queue_article` MCP tool + raw HTML snapshot metrics (`tests/agents/test_archive_ingest_pipeline.py`, `tests/agents/test_archive_raw_html_snapshot.py`) | Grafana wiring added (ingest/raw_html dashboard) and nightly canary/CI checks in place; still expand fixture coverage and DB visibility gates |
| 3. Parsing & structure extraction | 🟡 Fixtures in CI | Stage B pipeline now adopts a Crawl4AI-first approach for profile-driven crawls (see `crawl_and_scrape_stack.md`); Trafilatura remains the robust fallback for generic extraction. Deterministic checks live in `tests/parsing/test_canary_articles.py` with `tests/fixtures/canary_articles/` | Need broader fixture coverage (authors/dates edge cases) and linkage to normalized feed |
...
| 7. Observability & harness | 🟢 Dashboards+alerts seeded | Grafana JSON + provisioning (`docs/grafana/`, including `editorial-harness-dashboard.json`) now mirrors the curated dashboards tracked under `monitoring/dashboards/generated/`. Ingest/raw_html dashboard was added to the generated set and a scheduled canary-run (CI) validates the full pipeline nightly. | Next: refine alerts and add stricter CI gating for canary KPIs |
...
1. Stage 0 — Setup & baseline
   - **Status:** ✅ Complete. Dev setup instructions (`docs/dev-setup.md`) and smoke test `tests/smoke/test_stage0_env.py` validate the canonical conda env, compose stack, and required repo files. Canary URL list lives in `scripts/dev/canary_urls.txt` to keep runs deterministic.
...
2. Stage 1 — Crawl & fetch verification
   ...
   - Verify fetchers write raw HTML and metadata. Raw HTML references (`raw_html_ref`) are generated, but confirmation that `archive_storage/raw_html` holds every payload still depends on the archive agent wiring.
...
4. Stage 3 — Parsing & structure extraction
   - **Status:** 🟡 Fixtures in CI. Crawl4AI-first extraction is preferred for profile-driven crawls while Trafilatura (with readability/jusText fallbacks) remains the reliable generic fallback. Deterministic assertions now live in `tests/parsing/test_canary_articles.py` powered by `tests/fixtures/canary_articles/` (promoted from `output/canary_*`).
...
4. Stage 4 — Reasoning, fact-check and editorial agents
   ...
   - Agent chain produces a draft/brief/fact-check bundle from live records, persists traces back to `articles` (fact_check_trace/synth_trace), and emits acceptance metrics; the nightly workflow `.github/workflows/editorial-harness.yml` plus `scripts/dev/bootstrap_editorial_harness_db.py` keeps the dry-run exercising DB/Chroma while we plumb stored drafts into the publisher checklist (see `docs/editorial_harness_runbook.md`).
...
1. **Automate parsing validation (initial suite done):** Deterministic fixtures now live under `tests/fixtures/canary_articles/` and are exercised via `tests/parsing/test_canary_articles.py`; extend coverage (authors/publish dates) and wire a nightly refresh job for new canaries. A PR-level canary smoke workflow (`.github/workflows/canary-pr.yml`) now runs parsing fixture checks on pull requests to provide fast early feedback — consider enabling this job as a required status check in branch protection rules.
...
- Grafana dashboard fragment `docs/grafana/publisher-dashboard.json` was added to show publish success/failure and latency.
 - Grafana dashboard fragment `docs/grafana/publisher-dashboard.json` was added to show publish success/failure and latency.
 - Grafana provisioning now includes the `ingest_archive_dashboard.json` under `monitoring/dashboards/generated/` so Stage1/2 counters are picked up by production provisioning.
 - CI: Added `.github/workflows/diagrams-ci.yml` to run the diagram renderer (mermaid-cli if available, with conda-first librsvg/ImageMagick fallbacks) and upload generated assets as build artifacts for PRs.
 - CI: Added `.github/workflows/canary-refresh.yml` — a scheduled nightly run of the canary pipeline (crawl -> normalize -> parse -> editorial -> publish) that validates canary KPI metrics and uploads artifacts for investigation.
...
- CI: added `/.github/workflows/e2e-publisher.yml` (e2e publisher test) and `/.github/workflows/editorial-harness-publish-sandbox.yml` (sandboxed harness publish) to exercise publish flows safely.
 - CI: added `/.github/workflows/e2e-publisher.yml` (e2e publisher test) and `/.github/workflows/editorial-harness-publish-sandbox.yml` (sandboxed harness publish) to exercise publish flows safely.
 - CI: added `/.github/workflows/diagrams-ci.yml` (diagram rendering + artifacts) and `/.github/workflows/canary-refresh.yml` (scheduled canary runs + artifacts and KPI validation).
- CI: added `/.github/workflows/e2e-publisher.yml` (e2e publisher test) and `/.github/workflows/editorial-harness-publish-sandbox.yml` (sandboxed harness publish) to exercise publish flows safely.
