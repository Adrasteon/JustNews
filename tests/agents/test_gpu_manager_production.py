...
def test_request_allocation_already_allocated():
    ...
    mgr._allocations['a1'] = types = type('T', (), {'status': 'active', 'gpu_device': 'cuda:0', 'allocated_memory_gb': 2.0, 'batch_size': 4})()
...
def test_allocate_and_release_success(monkeypatch):
    ...
    status = GPUStatus(device_id='cuda:0', device_type='cuda', total_memory_gb=16.0, used_memory_gb=2.0, free_memory_gb=14.0, utilization_percent=10.0, temperature_c=50.0, power_draw_w=80.0, is_healthy=True)
    mgr.health_monitor = FakeHealthMonitor(['cuda:0'], {'cuda:0': status})

    res = mgr.request_gpu_allocation('agentY', 4.0, preferred_device='cuda:0', model_type='embedding')
...
def test_batch_size_heuristics(monkeypatch):
    ...
    status = GPUStatus(device_id='cuda:0', device_type='cuda', total_memory_gb=16.0, used_memory_gb=6.0, free_memory_gb=10.0, utilization_percent=20.0, temperature_c=45.0, power_draw_w=70.0, is_healthy=True)
    mgr.health_monitor = FakeHealthMonitor(['cuda:0'], {'cuda:0': status})
