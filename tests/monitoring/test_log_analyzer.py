...
def test_analyze_error_rates(tmp_path):
    ...
    async def fake_query(query):
        ...
        if getattr(query, "filters", None) and query.filters.get("level") == "ERROR":
            return QueryResult(
                entries=entries[:2], total_count=2, has_more=False, query_time_ms=1
            )
        return QueryResult(
            entries=entries, total_count=3, has_more=False, query_time_ms=1
        )
    ...
    result = asyncio.run(
        analyzer.analyze_logs(
            AnalysisType.ERROR_RATE_ANALYSIS, time_range=(now - timedelta(hours=1), now)
        )
    )
...
def test_detect_anomalies_new_error_pattern(tmp_path):
    ...
    res = asyncio.run(
        analyzer.analyze_logs(
            AnalysisType.ANOMALY_DETECTION, time_range=(now - timedelta(hours=1), now)
        )
    )
