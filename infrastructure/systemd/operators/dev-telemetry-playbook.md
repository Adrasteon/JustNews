...
This playbook explains how operators can safely enable, verify, and manage the `dev` telemetry stack across developer/test hosts. The goal is to make it easy to enable the telemetry stack on many hosts in a repeatable and reversible way without affecting production systems.
...
Port conflicts on developer machines are expected if an existing OpenTelemetry collector or other telemetry process is already active. To avoid surprising failures the canonical startup checks for port conflicts and will skip starting the repo's dev telemetry stack if any canonical ports are already bound on the host. This prevents noisy docker-compose bind errors.
...
2. Set the opt-in environment variable in `/etc/justnews/global.env` (create if missing):

```ini
ENABLE_DEV_TELEMETRY=true
```

3. Option A: Use canonical startup script

```bash
sudo ./infrastructure/systemd/canonical_system_startup.sh
```

The canonical script will check `ENABLE_DEV_TELEMETRY` and attempt to start the repo's `dev-docker-compose.yaml` via docker compose.

3. Option B: Use systemd-managed unit
...
This installs and starts a systemd unit that runs the repository compose file. Use `systemctl status justnews-dev-telemetry.service` to inspect state.
...
```bash
sudo systemctl disable --now justnews-dev-telemetry.service
sudo rm /etc/systemd/system/justnews-dev-telemetry.service
