...
def allow_request_redis(request: Request, max_requests: int = 10, window_seconds: int = 60) -> bool:
...
def rate_limiter_dependency(request: Request, max_requests: int = 10, window_seconds: int = 60):
    ...
    ok = allow_request_redis(request, max_requests=max_requests, window_seconds=window_seconds)
    if not ok:
