...
def make_coordinator(monkeypatch):
    ...
    monkeypatch.setattr(OnTheFlyTrainingCoordinator, '_training_loop', lambda self: None)

    # Create coordinator with small thresholds for tests
    coord = OnTheFlyTrainingCoordinator(update_threshold=2, max_buffer_size=10, performance_window=5, rollback_threshold=0.05)
...
def test_add_training_example_buffers_and_persists(monkeypatch):
    ...
    coord.add_training_example(agent_name='scout', task_type='sentiment', input_text='x', expected_output=1, uncertainty_score=0.9)

    assert len(coord.training_buffers['scout']) == 1

    # Add another and ensure buffer keeps both
    coord.add_training_example(agent_name='scout', task_type='sentiment', input_text='y', expected_output=0, uncertainty_score=0.95)
...
def test_add_prediction_feedback_adds_only_high_uncertainty(monkeypatch):
    ...
    coord.add_prediction_feedback(agent_name='analyst', task_type='sentiment', input_text='a', predicted_output=0, actual_output=1, confidence_score=0.2)
    assert len(coord.training_buffers['analyst']) == 1

    # Low uncertainty and correct prediction -> no add
    coord.add_prediction_feedback(agent_name='analyst', task_type='sentiment', input_text='b', predicted_output=1, actual_output=1, confidence_score=0.95)
    assert len(coord.training_buffers['analyst']) == 1
...
def test_update_agent_model_performance_drop_triggers_rollback(monkeypatch):
    ...
    for i in range(4):
        coord.add_training_example('scout', 'sentiment', f't{i}', expected_output=i % 2, uncertainty_score=0.9)

    # Make perform update succeed
    monkeypatch.setattr(OnTheFlyTrainingCoordinator, '_perform_model_update', lambda self, agent, examples: True)
    ...
    monkeypatch.setattr(OnTheFlyTrainingCoordinator, '_evaluate_agent_performance', fake_eval)
