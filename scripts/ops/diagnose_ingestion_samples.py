...
def check_article_exists(conn, url: str, normalized: str | None, urlhash: str | None):
    ...
    out = {}
    try:
        cursor.execute('SELECT id, url, normalized_url, url_hash, created_at FROM articles WHERE url = %s LIMIT 1', (url,))
        row = cursor.fetchone()
        if row:
            out['by_url'] = {'id': row[0], 'url': row[1], 'normalized_url': row[2], 'url_hash': row[3], 'created_at': row[4].isoformat() if getattr(row[4], 'isoformat', None) else str(row[4])}
        else:
            out['by_url'] = None

        if normalized:
            cursor.execute('SELECT id, url, normalized_url, url_hash, created_at FROM articles WHERE normalized_url = %s LIMIT 1', (normalized,))
            row = cursor.fetchone()
            out['by_normalized'] = {'id': row[0], 'url': row[1], 'normalized_url': row[2], 'url_hash': row[3], 'created_at': row[4].isoformat() if getattr(row[4], 'isoformat', None) else str(row[4])} if row else None
        else:
            out['by_normalized'] = None

        if urlhash:
            cursor.execute('SELECT id, url, normalized_url, url_hash, created_at FROM articles WHERE url_hash = %s LIMIT 1', (urlhash,))
            row = cursor.fetchone()
            out['by_hash'] = {'id': row[0], 'url': row[1], 'normalized_url': row[2], 'url_hash': row[3], 'created_at': row[4].isoformat() if getattr(row[4], 'isoformat', None) else str(row[4])} if row else None
        else:
            out['by_hash'] = None
...
def main():
    ...
    parser.add_argument('--sample', '-n', type=int, default=10, help='Number of recent raw_html files to sample')
    parser.add_argument('--post', action='store_true', help='POST payload to memory agent /ingest_article for each sample (will cause side effects).')
    parser.add_argument('--post-url', default='http://127.0.0.1:8007/ingest_article', help='Memory agent ingest endpoint')
    ...
    report = []
    for f in files:
        sample = {'file': str(f), 'sampled_at': datetime.fromtimestamp(f.stat().st_mtime).isoformat()}
        ...
        if url:
            ...
            if conn:
                sample['db_check'] = check_article_exists(conn, url, normalized, urlhash)
            else:
                sample['db_check'] = None

            if args.post:
                payload = {'article_payload': {'url': url, 'normalized_url': normalized, 'content': f.read_text(errors='ignore')[:2000]}}
                try:
    ...
    found = any((r.get('db_check') and (r['db_check'].get('by_url') or r['db_check'].get('by_normalized') or r['db_check'].get('by_hash'))) for r in report)
    if found:
