...
class ComplianceService:
    ...
    async def record_consent(self, user_id: int, purpose: str,
        ...
        """
        try:
            consent_id = f"consent_{datetime.now(UTC).timestamp()}_{secrets.token_hex(4)}"
    ...
    async def _cleanup_old_events(self) -> None:
        ...
        while True:
            try:
                ...
                cutoff_date = datetime.now(UTC) - timedelta(days=self.compliance_config.audit_retention_days)
    ...
    async def _cleanup_expired_consents(self) -> None:
        ...
        while True:
            try:
                ...
                cutoff_date = datetime.now(UTC) - timedelta(days=self.compliance_config.consent_retention_days)
