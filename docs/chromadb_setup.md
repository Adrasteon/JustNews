...
export CHROMADB_HOST=localhost export CHROMADB_PORT=3307
...
2) Diagnose & auto-provision ---------------------------- Run the included diagnostic script to discover endpoints, test
connectivity, and optionally try to create the default tenant and `articles` collection (best-effort, not guaranteed on
all server versions):
