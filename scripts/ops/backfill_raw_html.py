...
RAW_DIR = Path(os.environ.get("JUSTNEWS_RAW_HTML_DIR", SERVICE_DIR / "archive_storage" / "raw_html"))
    ...
    payload = {
        ...
        'kwargs': {
            'article_payload': {
                'url': url,
                'content': html,
                'raw_html_ref': str(path.relative_to(SERVICE_DIR)) if path.is_relative_to(SERVICE_DIR) else str(path)
            },
            'statements': []
        }
    }
