...
def run_script(root: Path, apply: bool = False) -> str:
    cmd = ["python3", "scripts/codemods/replace_run_until_complete.py", "--root", str(root)]
    if apply:
        cmd.append("--apply")
...
def test_var_loop_no_new_event_loop_not_replaced(tmp_path: Path):
    ...
    assert "Applied replacements to 0 files" in out2 or "Applied replacements to 0" in out2
