...
spec2 = importlib.util.spec_from_file_location('security.encryption.service', encryption_path)
...
def test_generate_symmetric_key_and_encrypt_decrypt():
    ...
    key_id = asyncio.run(svc.generate_key(algorithm='AES-256', key_type='symmetric', usage='encrypt'))
...
def test_get_active_keys_and_status():
    ...
    async def _noop(*a, **k):
        return None
    svc._save_keys = _noop
    kid = asyncio.run(svc.generate_key())
