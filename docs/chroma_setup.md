...
PYTHONPATH=. conda run -n justnews-v2-py312 python scripts/print_db_config.py
...
PYTHONPATH=. CHROMADB_REQUIRE_CANONICAL=1 CHROMADB_CANONICAL_HOST=localhost CHROMADB_CANONICAL_PORT=3307 \
  conda run -n justnews-v2-py312 python scripts/chroma_diagnose.py --host localhost --port 3307 --autocreate

# If diagnosis shows missing tenant or collection, attempt to create them
PYTHONPATH=. conda run -n justnews-v2-py312 python scripts/chroma_bootstrap.py --host localhost --port 3307 --tenant default_tenant --collection articles
```
