...
- **Environment**: Stage B development host (`conda env justnews-v2-py312`)
...
| Stage B metrics emitting | Complete | Embedding counters/histogram verified via targeted pytest run. | `conda run -n justnews-v2-py312 python -m pytest tests/agents/crawler/test_extraction.py tests/agents/memory/test_save_article.py -q`
