...
1. Load the current crawl profile and exercise the standard variants:
   ```bash
   PYTHONPATH=. conda run -n ${CANONICAL_ENV:-justnews-py312} python scripts/dev/crawl4ai_profile_variants_probe.py \
     --domain bbc.co.uk \
     --max-articles 30
   ```
...
- Use the scheduler with the updated profile to mimic production:
   ...
   set -a && source global.env && set +a
    conda run -n ${CANONICAL_ENV:-justnews-py312} python scripts/ops/run_crawl_schedule.py \
