...
Use `scripts/ops/apply_synthesis_migration.sh` to safely apply the `004_add_synthesis_fields.sql` and `005_create_synthesized_articles_table.sql` migrations in staging/production. The script logs output to `logs/operations/migrations` and supports both MySQL/MariaDB and PostgreSQL connection strings.
